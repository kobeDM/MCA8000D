...
void save_root(const std::string& prefix, const std::string& out_dir,
    ...
    TFile f((out_dir + prefix + "3He_data.root").c_str(), "RECREATE");
...
void fit_and_plot(const std::string& prefix, const std::string& out_dir,
    ...
    c.SaveAs((out_dir + prefix + "3He.png").c_str());

    // --- ガウシアン＋線形フィット ---
    TF1 *fitFunc_low = new TF1("fitFunc_low",
        "[0]*exp(-0.5*((x-[1])/[2])**2) + [3] + [4]*x", 660, 860);
    ...
    // --- 初期値設定 ---
    double p0_amplitude_low = 5;
    double p1_mean_low      = 740;
    double p2_sigma_low     = 20;
    double p3_slope_low     = -200;
    double p4_offset_low    = 300;
    ...
    TF1 *fitFunc_high = new TF1("fitFunc_high",
        "[0]*exp(-0.5*((x-[1])/[2])**2) + [3] + [4]*x", 1500, 1900);
    ...
    // --- 初期値設定 ---
    double p0_amplitude_high = 5;
    double p1_mean_high      = 2600;
    double p2_sigma_high     = 50;
    double p3_slope_high     = -50;
    double p4_offset_high    = 100;

    fitFunc_high->SetParameters(p0_amplitude_high, p1_mean_high, p2_sigma_high, p3_slope_high, p4_offset_high);


    // --- デバッグ出力 ---
    std::cout << "Amplitude_low = " << fitFunc_low->GetParameter(0) << std::endl;
    std::cout << "Mean_low      = " << fitFunc_low->GetParameter(1) << std::endl;
    std::cout << "Sigma_low     = " << fitFunc_low->GetParameter(2) << std::endl;
    std::cout << "Amplitude_high = " << fitFunc_high->GetParameter(0) << std::endl;
    std::cout << "Mean_high      = " << fitFunc_high->GetParameter(1) << std::endl;
    std::cout << "Sigma_high     = " << fitFunc_high->GetParameter(2) << std::endl;
    ...
    // c.SaveAs((out_dir + prefix + "3He_fit.png").c_str());
